...
export const checkBookingConflict = async (providerId: string): Promise<BookingConflict> => {
  try {
    ...
    const response = await fetch(`${base}/api/bookings/active/${providerId}`, {
      method: 'GET',
      headers: {
    ...
    if (response.ok) {
      ...
      return {
        hasConflict: data.data.hasActiveBookings,
        activeBookings: data.data.activeBookings || [],
        conflictReason: data.data.hasActiveBookings ? 'You have active bookings with this provider' : undefined
      };
    } else {
