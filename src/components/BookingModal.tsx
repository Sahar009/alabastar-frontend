...
export default function BookingModal({ provider, isOpen, onClose, onBooked }: BookingModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setSuccessData((result?.data as BookingSuccessData) || null);
    } catch (e) {
      console.error(e);
      // @ts-expect-error typed broadly due to RTK error shape
      const message = e?.data?.message || e?.error || 'Failed to create booking';
      toast.error(message);
    }
  };

  if (successData) return <BookingSuccessModal data={successData} onClose={() => { setSuccessData(null); onClose(); }} />;
        ...
        <div className="p-4 space-y-4 overflow-y-auto max-h-[60vh]">
          ...
          <div>
            ...
            {servicesLoading ? (
              ...
              <select value={selectedServiceId} onChange={(e) => setSelectedServiceId(e.target.value)} className="w-full p-3 border border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-slate-100">
          ...
          {/* Time slots */}
          <div>
            <label className="block text-sm font-medium mb-2 text-slate-700 dark:text-slate-300">Available Slots</label>
            {availabilityLoading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {availableSlots.map((slot) => (
                  <button key={slot.time} onClick={() => setSelectedSlot(slot.time)} className={`px-3 py-2 rounded-lg border text-sm cursor-pointer ${selectedSlot === slot.time ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
